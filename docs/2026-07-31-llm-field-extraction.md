...
| W3 | ✅ **CLOSED 2026-08-03 — after it caused a production failure, which is the lesson worth keeping.** Carried forward from Change 1 as a documentation-only risk: "no converted-winner test of `resolveLiveRecord`; every test that routes a competing-broker or repeat submission onto 'the winning Lead' uses an unconverted Lead as the winner." The untested case was not merely unproven, it was BROKEN — the platform rejects a lookup SET to a converted Lead (`CANNOT_UPDATE_CONVERTED_LEAD`), so the first repeat on a converted winner threw a `DmlException` in branch (b) and the email lost its audit row and its Task target (staging row `a0aiw000000OCckAAG`). Fixed via the split winner anchor (`Winning_Lead__c` / `Winning_Opportunity__c`, exactly one populated) plus the widened repeat-detection filter; see `docs/2026-07-31-competing-broker-no-lead.md`'s W3 row for the full note. |
