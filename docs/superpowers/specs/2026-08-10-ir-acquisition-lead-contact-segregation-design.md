...
| 🔴 `viewAllRecords` / `modifyAllRecords` on Account, Contact or Lead in any permission set defeats sharing entirely | **PRESENT, AND IT MAKES THIS FEATURE INERT AS DESIGNED.** An earlier version of this row claimed "verified absent" — that was **wrong**, produced by a truncated grep. Measured 2026-08-10: `DPEG_Contact_Edit` grants **View All on Contact**, and `DPEG_Acquisition_Edit` grants **View All on Lead**. Both are held by the Junior Analyst PSG (and `DPEG_Contact_Edit` by Property Management). View All bypasses OWD and every sharing rule **for read**, so after the OWD flip an acquisitions analyst would still see every IR Lead and Contact. **These grants must be REPLACED — not merely deleted — before the boundary is real: they exist to give the team owner-independent read, which the §4 criteria-based rules now provide, scoped by record type.** See the new remediation task in the plan; it is a prerequisite of Task 12, not a follow-up to it. |
