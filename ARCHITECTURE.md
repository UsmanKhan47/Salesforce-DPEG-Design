...
### Two home-page deal cards, deliberately — `recentOpportunities` and `topLargestDeals`

Added 2026-08-16 (FSD tranche 2, §24.2). The Acquisition home page carries **two five-row deal
cards, and they are not duplicates**:

| Bundle | Ranks by | Apex | Question it answers |
| --- | --- | --- | --- |
| `c/recentOpportunities` | `Asking_Price__c` | `OpportunityFunnelController.getRecentOpportunities` → `OpportunitySelector.selectTopByAskingPrice` | what the SELLER wants for the asset |
| `c/topLargestDeals` | **`Amount`** | `TopDealsController.getTopDeals` → `OpportunitySelector.selectTopByAmount` | what the DEAL is worth to DPEG |

🔴 **THE TRANCHE-2 DESIGN PROPOSED THE OPPOSITE — adding a stage filter to `selectTopByAskingPrice`
and relabelling the existing card — AND THAT WAS OVERRULED BY THE USER (2026-08-16).**
`recentOpportunities`, its controller method, its selector method, its label and its placement are
UNTOUCHED. A deal can top one list and not the other, so a merged card would silently answer only
one of two questions while appearing to answer both.

⚠ **THIS IS NOT THE `c/transactionAdvanceStage` MISTAKE.** §5 records that bundle being created and
DELETED the same day for being byte-identical to `c/advanceRecordStage` below the comments, and
states the rule: *a copy carrying only a different header is not a split — it is a second file that
must now receive every fix the first one gets.* These two differ in the fact they rank on, in their
Apex wire, and in two of six columns (Amount + Close Date vs. Asking Price + NOI). That is a genuine
split.

⚠ **`Opportunity.Amount` IS `permissionable` AND IS GRANTED BY NO PERMISSION SET IN THIS REPO** (as
at 2026-08-16 it appears only in `profiles/**`, which is `.forceignore`d and never deploys).
`selectTopByAmount` is `WITH USER_MODE` — correctly, because it backs a read a human asked for — and
USER_MODE throws on the WHOLE ROW for ONE inaccessible field, so a persona without that grant gets
the card's error banner rather than a blank column. That failure is LOUD by design; the fix is a
permission-set grant on `DPEG_Acquisition_Edit` / `_View` / `DPEG_Opportunity_View`, not a mode change.

⚠ **Stage/deal-type pill colours in both cards are INLINE styles built in JS, not `.css`,** and that
is structural: `c/listDatatable` is a `lightning/datatable` SUBCLASS, and a datatable subclass cannot
reach its custom cell templates from a stylesheet (see that bundle's own comment). Moving them into
the `.css` file silently stops applying them. The SLDS 2 tokens govern everything a stylesheet
does reach.
