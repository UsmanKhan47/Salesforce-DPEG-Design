...
34 custom objects (was 33 until 2026-08-17, when `Property_Package__c` was added). Grouped by the module that owns them. `Parent (lookup)` is the object's own relationship graph and is authoritative. Per §6, **add a row here whenever a custom object is created.**

A `—` in _Purpose_ means the object's `<description>` in its `.object-meta.xml` is unset. **22 of 34 are unset** — the COUNT of unset objects is unchanged from the 2026-07-15 filesystem verification (the previous count of 21 was wrong against every measure); only the denominator moved, because `Property_Package__c` shipped WITH a `<description>` and so did not become a 23rd. Where a description exists it is the authoritative source and is condensed here.
...
| ------------------------------------ | ------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `Property__c` **†**                  | — (graph root)                                    | The acquisition target. Created on lead conversion by `LeadConvertService`. **Extended 2026-08-11 (SharePoint deal folder):** it carries the feature's ENTIRE result state in four new fields — `SharePoint_Folder_ID__c` (Text 255), `SharePoint_Folder_URL__c` (Url), `SharePoint_Folder_Status__c` (restricted Picklist: Pending / Created / Failed / Skipped) and `SharePoint_Folder_Error__c` (Long Text 4096). 🔴 **The state lives on the PROPERTY, not the Opportunity, and that is the idempotency key**: two deals closing against one property share one folder, matching `PropertyAssetService`. `SharePoint_Folder_ID__c` non-blank means ZERO callouts on every path. `SharePoint_Folder_Status__c` is a THIRD work queue in this repo, after `Inbound_Email_Staging__c`'s two — `Pending` and `Failed` are `DealFolderSweepBatch`'s queue, `Created` and `Skipped` are terminal and excluded by the locator. ⚠ A deal with a null `Property__c` has nowhere to record any of this and is skipped silently (`DealFolderService` residual R1), and a Property never claimed by the trigger has a NULL status, which is invisible to the sweeper (residual R5 — historical deals). **Extended 2026-08-16 (FSD tranche 2 — the MARKET DATA LAYER):** it is the **LIVE** half of a deliberate two-layer split, and carries the whole Placer.ai / CoStar block — the pre-existing metrics (`Placer_URL__c`, `CoStar_URL__c`, `Monthly_Visits__c`, `Trade_Area_Population__c`, `Peak_Hour__c`, `YoY_Growth__c`, `Comp_Monthly_Visits__c`, `Comp_Sales__c`, `Visitor_Demographics__c`, `Market_Rent_PSF__c`, `Market_Cap_Rate__c`, `Occupancy_Rate_Market__c`, `Days_On_Market_Avg__c`, `Placer_Fetch_Status__c`) plus 13 added in that tranche (4 Placer ranks, 4 CoStar metrics, and the per-provider control fields `*_Data_Source__c` / `*_Last_Synced_DateTime__c` / `CoStar_Fetch_Status__c`). 🔴 **LIVE ON THE PROPERTY, SNAPSHOT ON THE OPPORTUNITY — the split is what makes the eventual ASB integration a DATA-SOURCE FLIP rather than a rebuild.** Market data is a fact about the PROPERTY (two deals can target one), so it belongs here; the underwriting freeze must be PER-DEAL, so it lives on `Opportunity.Market_Data_Snapshot__c` — see `MarketDataSnapshotService` in §2. Freezing this layer instead would freeze it for the OTHER deal too and would block the refresh the design exists to enable. ⚠ **`Placer_Data_Source__c` / `CoStar_Data_Source__c` HAVE NO CONSUMER TODAY AND THAT IS CORRECT, NOT AN OVERSIGHT.** They are PROVENANCE LABELS, not switches: nothing branches on them and nothing should until an ASB spoke exists, because building a switch with only one branch is how a feature ships inert. Their value arrives the day the spoke lands and the flip is a data change (post-deploy gate G9). ⚠ **`CoStar_Pct_Leased__c` is DELIBERATELY separate from `Occupancy_Rate_Market__c`** — leased ≠ occupied in CRE (a signed-but-not-yet-occupied space is leased and not occupied), and folding them would silently redefine a live field: the `Unit__c` Text-vs-MasterDetail defect class (§1 rule 9 prohibition 1). ⚠ **`Opportunity.Market_Cap_Rate__c` and `My_Cap_Rate__c` ARE NOT PART OF THIS LAYER AND MUST NOT BE WRITTEN BY IT** — they are owned by `flows/Underwriting_Opp_Sync`, which copies the `Underwriting__c` values onto them on every Underwriting save, so any market value written there is silently overwritten. They are an UNDERWRITING mirror, not a market one. |
...
| Object                          | Parent (lookup)                                      | Purpose                                                                                                                          |
| -------------------------------- | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `Property_Registry__c`           | `Lead` (`Winning_Lead__c`)                            | Backend-only claim ledger. One row per distinct property; the unique, case-insensitive `Property_Key__c` enforces race-safe first-come-first-served registration at the database level. **⚠ A CLAIM NOW EXPIRES (2026-08-17, FSD tranche 3 item 1).** `Registered_DateTime__c` means *"when the CURRENT winner claimed it"* — it is RESET on every adoption — and a registration older than `PropertyMatchingService.PROTECTION_WINDOW_DAYS` (90) **releases the property** to the next broker. 🔴 **This was a DEFECT FIX, not a new feature**: two of the three claim reads (the FUZZY registry scan and REPEAT detection) were already windowed at 90 days while the EXACT-key read was not, so past day 90 the SAME property produced OPPOSITE outcomes depending only on how the second broker spelled the address — identical wording kept the incumbent for ever, different wording won it outright and minted a SECOND registry row for one building. 🔴 **A claim whose `Winning_Lead__c` has been CONVERTED NEVER EXPIRES**, and that is structural rather than a courtesy: it is the only reading under which the adoption `update` can never touch a row whose lookup holds a converted Lead, i.e. the only one that satisfies `PropertyClaimService`'s own written prohibition (see §2) without giving this object the same split anchor `Competing_Broker_Submission__c` has. ⚠ **Stated residual — a converted-then-DEAD deal protects its property for ever** (`IsConverted` stays true); the remedy needs no code, only deleting the registry row by hand, which re-opens the key through the existing orphan-adoption path. ⚠ **Expiry is LAZY and therefore INVISIBLE until someone resubmits the property** — no state changes, so there is no "expired claims" list view unless the optional `Protection_Expiry_Date__c` formula (admin item A1) is built. |
...
| `Property_Package__c`            | — (graph root; three CHILDREN point at it — `Lead`, `Opportunity`, `Property__c`, each via a `Property_Package__c` lookup with `deleteConstraint SetNull`) | ONE inbound broker email that named TWO OR MORE addressable properties, grouping every record that email produced so the Acquisitions team can work a package deal as a package. Added 2026-08-17. Created ONLY by the Broker Protection pipeline (`PropertyPackageService`, called once per email from `ExtractAddressQueueable` PRE-LOOP), never by hand. 🔴 **IT IS A VISIBILITY SURFACE OVER AN EXISTING DERIVATION, NOT A NEW ONE.** The pipeline already derived the grouping correctly and recorded it as TEXT in `Inbound_Email_Staging__c.Routed_Record_Ids__c` — *"the only place the full property-to-record mapping exists"* — on a `Private`-OWD backend object the Acquisitions team cannot see. Nothing about the routing tree, the claim engine or the Task contract changed. **The gate is `workList.size() > 1`** — the DE-DUPLICATED, ADDRESSABLE, CAPPED count, i.e. records that will EXIST (Decision E): a single-property email creates ZERO packages, pinned by the `== 7` single-property DML EQUALITY in `ExtractAddressQueueableTest`, which is the permanent falsifier. `Lead.Property_Package__c` is stamped AT INSERT (Decision C1, so no Lead is ever re-updated); `Opportunity` and `Property__c` receive it at conversion from `LeadConvertService`. 🔴 **RESIDUAL R1 — `Opportunity` NOW CARRIES TWO UNRELATED PORTFOLIO CONCEPTS AND THEY MUST NEVER BE MERGED, CROSS-POPULATED OR REPORTED AS ONE:** this object is PIPELINE-DERIVED (one email → many Leads/Opportunities/Properties), while the dormant, MANUAL `Portfolio_Deal__c` self-lookup + `Is_Portfolio_Parent__c` + `Bundle_LOI__c` + the `Portfolio Deal` stage describe one SELLER bundling several deals under a single LOI. They answer DIFFERENT QUESTIONS, have NO relationship, and no Apex, Flow or FlexiPage reads or writes the second one at all. **NO AUTOMATED TEST CAN DETECT A DIVERGENCE — this row, the §2 service row, and the comment at the `applyDealFields` assignment are the only control.** Decision A2, user-directed. ⚠ `sharingModel = ReadWrite` (Decision F1), matching `Property_Registry__c` / `Competing_Broker_Submission__c`: that choice DELETES the whole `private without sharing` inner-class failure class for every reader, which is why `PropertyPackageSelector` has none and must not acquire one. ⚠ All three lookups are `SetNull`, so deleting a package NEVER cascades into deal records. ⚠ A MEMBERLESS or one-member package is an accepted residual (design R6), not a defect — it records that a multi-property email arrived and its properties failed to route, and `Routed_Outcomes__c` says which. |
| `Content_Publication_Budget__c` **(Custom Setting — NOT one of the 34 custom objects)** | — (hierarchy custom setting; org default only) | **The FIRST Custom Setting in this repo** (added 2026-08-06). Holds the file pipeline's daily publication counter and circuit-breaker state (`Publication_Count__c`, `Window_Start_DateTime__c`, `Is_Suspended__c`, `Suspended_Until_DateTime__c`). Chosen over a custom object because `getOrgDefaults()` costs **0 SOQL**, so consulting it does not move `AttachmentPersistQueueable`'s pinned query budget. ⚠ **Custom-setting DATA is not deployable** — there is no org-default row in source control and there never will be; `ContentPublicationBudget` creates it at runtime on first use. Only the ORG DEFAULT is ever read; a per-user override would split one org-wide counter into several partial ones. |
...
As of 2026-08-16 (FSD tranche 2, the market-data freeze) the repo has **33 `WITH SYSTEM_MODE`
queries across 19 selector classes** — the count of QUERIES moved and the count of CLASSES did not,
because both recent entrants landed on the incumbent `OpportunitySelector`, taking it from one
`SYSTEM_MODE` method to three (`selectCallForOffersTargetsByIds`, `queryCallForOffersAlerts` and now
`selectMarketSnapshotSourceByIds`) — and **note that all three argue their mode from a DIFFERENT
fact, which is exactly why the count is not a conformance test.** ⚠ **THE PUBLISHED COUNT WAS
ALREADY STALE BEFORE THIS TRANCHE TOUCHED IT, AND THAT IS WORTH KNOWING RATHER THAN QUIETLY
CORRECTING:** the 2026-08-14 call-for-offers alert job added `queryCallForOffersAlerts` and updated
`OpportunitySelector`'s own header (which correctly reads "TWO are `WITH SYSTEM_MODE`") without
updating this line — the same stale-summary failure shape recorded for `LoiSelector` and
`NdaSelector` immediately above. ⇒ **The authoritative inventory is and remains the selector class
headers; treat this running number as a narrative aid only.**
⚠ **2026-08-17 (the multi-property email package) ADDED THREE SELECTOR METHODS AND MOVED THIS COUNT
BY ZERO — recorded so the absence reads as a DECISION rather than as an omission.**
`PropertyPackageSelector.selectWithMembersById` (a brand-new class: the FIRST and only
`Property_Package__c` SOQL in the application), `LeadSelector.selectPackageAnchorById` and
`OpportunitySelector.selectPackageAnchorById` are all `WITH USER_MODE`, and all three are the case
this table exists to CONTRAST against: they back a card on a record page a HUMAN ALREADY OPENED, at
their request. Neither SYSTEM_MODE argument applies — there is no automation to silently disable (the
"feature" is the human's own page view), and the day-one no-FLS-on-a-new-custom-field problem is
answered by PROVISIONING, which is why the grants are declared in-file in `DPEG_Acquisition_Edit`,
`DPEG_Acquisition_View` and `Broker_Protection_Access` in the same change. No `without sharing` inner
class either: `Property_Package__c` is `sharingModel = ReadWrite`, so there is no boundary to escape.
🔴 **Do not "correct" any of the three toward SYSTEM_MODE** — `PropertyPackageControllerTest`'s
bare-user test stops proving anything the moment one of them stops throwing.
...
| **Market-data freeze at underwriting approval** (FSD tranche 2, 2026-08-16) | 🔴 **THE APPROVING PRINCIPAL** — `flows/Opportunity_UW_Approved_Notify` is approval-triggered, and `DPEG Principal PSG` grants Read and deliberately NOT Edit on `Opportunity` (measured) | **`OpportunitySelector.selectMarketSnapshotSourceByIds`** (← `MarketDataSnapshotService.snapshotMarketData`) — **NO `without sharing` inner class, and that is the decision this row exists to record** | 🔴 **MODE AND SHARING REACH OPPOSITE ANSWERS HERE, AND THE SHARING HALF IS THE ONE MOST LIKELY TO BE "FIXED" INTO A DEFECT.** MODE: every `Placer_*` / `CoStar_*` field in the SELECT is a Metadata-API-deployed custom created in this same tranche, so it arrives with NO field permissions for ANY profile, System Administrator included; under `USER_MODE` the read throws `System.QueryException: No such column` — an FLS denial wearing a schema error — **inside an approval-triggered flow, so it does not merely disable the snapshot, it takes the principal's approval down with it.** That is the `ApprovalAuditService` incident of 2026-08-05 reproduced one action later in the same flow, and it is the same shape as the Disposition-NDA auto-create row above. The read is also one the running user never ASKED for: a principal pressed Approve, and the platform is copying market data on their behalf. SHARING: **deliberately NOT escaped, on a STRUCTURAL argument rather than an optimistic one.** The ROOT object is the `Opportunity` the approver has just approved — visible to them by construction, and `Opportunity` OWD is `ReadWrite` besides — and `Property__r.*` is a PARENT-LOOKUP TRAVERSAL, which is not sharing-filtered (sharing filters a query's ROOT rows). 🔴 **WHICH IS PRECISELY WHY THIS METHOD LIVES ON `OpportunitySelector` AND NOT ON `PropertySelector`:** `Property__c` is `sharingModel = Private` and its single sharing rule is an OWNER rule scoped to rows owned by the `Acquisition` QUEUE, which a `LeadConvertService`-created Property (owned by the converter) does not reach — so a ROOT-LEVEL `Property__c` read would return ZERO ROWS for a principal approver and the snapshot would be **SILENTLY EMPTY while `Market_Data_As_Of_Date__c` stamped**: a record asserting "these are the numbers the principals saw" and naming none, with nothing thrown and no test failing. ⚠ **NAMED FALLBACK, recorded rather than built** — if traversal ever DOES prove to be sharing-filtered, the remedy is a `private without sharing` inner class on `PropertySelector` holding only that query, mirroring `FolderCreationReads` / `AssetCreationReads`, and NEVER `without sharing` on a class backing a dozen user-facing reads. ⚠ The matching **DML** is `Database.update(..., false, AccessLevel.SYSTEM_MODE)` — a THIRD decision implied by neither of these two: the approver holds Read and not Edit on `Opportunity`, and both target fields are new customs with no FLS for anyone. ⚠ **`ApprovalAuditService`'s own comment records that `<runInMode>SystemModeWithoutSharing</runInMode>` on the flow did NOT lift the access mode** — the stamp still silently wrote nothing until `AccessLevel.SYSTEM_MODE` was stated AT THE DML. Do not rely on the flow's configuration. |
...
| `MarketDataSnapshotService`   | `flows/Opportunity_UW_Approved_Notify` (`@InvocableMethod`, action `Snapshot_Market_Data`, chained after `Stamp_Approval_Audit`) | **Freezes the deal's market data at underwriting approval** (FSD tranche 2, 2026-08-16, §18.3 / §18.4). Reads the LIVE market block off the deal's `Property__c` and composes one labelled, ordered text block onto `Opportunity.Market_Data_Snapshot__c` + `Market_Data_As_Of_Date__c`. 🔴 **THE TWO-LAYER SPLIT IS THE WHOLE DESIGN: LIVE on `Property__c`, SNAPSHOT on `Opportunity`.** Market data is a property of the PROPERTY (two deals can target one — `PropertyAssetService`'s idempotency key is the Property for exactly this reason), while the freeze must be PER-DEAL. 🔴 **FREEZE BY SNAPSHOT, NEVER BY LOCK — a `Market_Data_Locked__c` + validation-rule design was REJECTED and must not be reintroduced:** it freezes a Property shared by other, un-underwritten deals; it blocks the very ASB refresh §18.4 exists to enable; and §18.3's intent is EVIDENTIARY ("what did the principals see"), not a prohibition on the data moving. 🔴 **NO QUEUEABLE IS NEEDED, AND THAT IS A LIVE PROOF RATHER THAN AN INFERENCE:** the flow ALREADY carries `<runInMode>SystemModeWithoutSharing</runInMode>`, `Opportunity.Underwriting_Approval` declares `finalApprovalRecordLock = false`, and `ApprovalAuditService` ALREADY updates the Opportunity from inside this exact flow and works today — **contrast `LoiPrimaryStampQueueable` / `DispositionNdaStampQueueable`, which exist ONLY because their records ARE locked; do not copy them here.** ⚠ But `runInMode` alone is NOT sufficient: `ApprovalAuditService`'s own comment records that setting it did not lift the access mode, so the DML is `Database.update(list, false, AccessLevel.SYSTEM_MODE)` stated AT the statement. 🔴 **THE CATCH IS WIDE *AND* SELF-REPORTING, AND BOTH HALVES ARE LOAD-BEARING** — §2 records that a `DmlException`-only catch let a `TypeException` roll back an approval, AND that the widened catch ALONE would have been worse than the bug (the approval succeeds while the stamp stays silently blank). So it catches `Exception`, never rethrows, and **writes the reason INTO `Market_Data_Snapshot__c`** (`"Market data snapshot failed: <message>"`) while still stamping the as-of date, which is what makes the previously invisible failure both readable on the record and QUERYABLE. 🔴 **A RE-APPROVAL RE-SNAPSHOTS, DELIBERATELY, AND IT IS NOT FILL-IF-BLANK:** the flow's `doesRequireRecordChangedToMeetCriteria = true` means a second run can only follow a reject → re-approve cycle, i.e. a SECOND review of the data as it then stood — fill-if-blank would leave that review permanently misrepresented by the first approval's numbers, which is a false evidentiary claim rather than merely a stale one. Budget: **1 SOQL / 1 DML per invocation, CONSTANT in record count** (the failure path costs the SAME one DML — the self-report rides the statement the snapshot would have). ⚠ `List` in / `List` out per `.claude/rules/invocable-rule.md`: Flow batches record-triggered interviews, so a single-record signature would silently drop every record but the first. Residuals: **R1** a deal with a null `Property__c` gets the literal "no property linked" text, never a blank (`PropertyAssetService` R1's shape, made legible); **R2** deals approved BEFORE deploy day never snapshot (`DealFolderService` R5's shape); **R3** it records what the PROPERTY held, not what a principal read in a browser tab. |
...
| `PropertyPackageService`       | `ExtractAddressQueueable` (`createForEmail` PRE-LOOP; `stampRoutedOutcomes` from `finish()`) | Broker Protection: **the ONLY class that writes `Property_Package__c`** (added 2026-08-17) — the grouping for one inbound broker email that named two or more addressable properties. 🔴 **A VISIBILITY SURFACE OVER AN EXISTING DERIVATION.** The pipeline already produced this grouping and recorded it as TEXT on `Inbound_Email_Staging__c.Routed_Record_Ids__c`, a `Private`-OWD backend row the Acquisitions team cannot see; nothing about the routing tree, the claim engine or the Task contract changed. `createForEmail` is **ZERO SOQL** (a blind insert — the caller holds every value) and **exactly ONE DML**, `AccessLevel.SYSTEM_MODE` with `allOrNone = true`: every field is a Metadata-API-deployed custom, which arrives with NO field permissions for ANY profile including System Administrator, so a bare `insert` throws `System.TypeException` on the first multi-property email after deploy. ⚠ **`with sharing`, and NO `without sharing` inner class — none is needed and none must be added:** the object is `sharingModel = ReadWrite` (Decision F1), matching `Property_Registry__c` / `Competing_Broker_Submission__c`, which deletes the entire failure class the four existing `private without sharing` inner classes exist for. Narrowing that OWD is not a settings change; it re-opens that class for every reader. 🔴 **IT THROWS AND ITS CALLER IS WHAT MAKES THAT SAFE — the division is deliberate.** `ExtractAddressQueueable` owns the Lead and the registry claim, so it wraps the call and DEGRADES to "no grouping", recording the failure on `Error__c` (which stamps `Status__c = 'Error'`, putting the row in the human queue) rather than letting a failed VISIBILITY AID destroy a broker's first-come protection — the 2026-08-06 `ContentPublication` lesson applied at the caller, since the write cannot be moved to another transaction (the Leads need the Id in THIS one). `stampRoutedOutcomes` takes the OPPOSITE posture and says so at its own declaration: FAIL-SOFT, catching `Exception`, because it writes a CONVENIENCE COPY of text already permanent on the staging row. 🔴 **IT IS A SECOND DML STATEMENT AND THAT COULD NOT BE AVOIDED:** the routing audit exists only AFTER the per-property loop while the package Id is needed BEFORE it (Decision C1, so each Lead is stamped at insert with no Lead re-update), and Apex DML cannot write two SObject types in one statement — so the N=10 budget in `ExtractAddressQueueableTest` moved **43 → 45**, not to the 44 the design predicted. That +1 is flagged at the assertion and at the call site, with the one-line way to trade it back for Decision B2 named there. The two statements are **CONSTANT in property count** (2 at N=2, 2 at N=10); the single-property budgets (`≤20`, and the `== 7` EQUALITY) are UNCHANGED and are the permanent falsifier for Decision E. 🔴 **RESIDUAL R1 — `Opportunity` now carries TWO UNRELATED PORTFOLIO CONCEPTS.** `Property_Package__c` is pipeline-derived (one email → many Leads/Opportunities/Properties); the dormant, MANUAL `Portfolio_Deal__c` / `Is_Portfolio_Parent__c` / `Bundle_LOI__c` / `Portfolio Deal` stage describe one SELLER bundling several deals under a single LOI. They answer DIFFERENT questions, have NO relationship, and no automation touches the second at all. They must never be merged, cross-populated or reported as one. **NO AUTOMATED TEST CAN DETECT A DIVERGENCE** — the §1 object row, this row, and the comment at `LeadConvertService.applyDealFields`'s assignment are the only control (Decision A2, user-directed). ⚠ **`PropertyPackageSelector` (every READ) is `WITH USER_MODE` DELIBERATELY and is NOT in the automation-path table above** — it backs a card on a record page a human already opened, which is the case that table exists to contrast against; a future reader must not "correct" it toward SYSTEM_MODE. The two ANCHOR reads live in `LeadSelector.selectPackageAnchorById` and `OpportunitySelector.selectPackageAnchorById`, because SOQL follows the object it queries; the package-rooted read returns BOTH member collections through relationship subqueries, so `PropertyPackageController` costs **2 SOQL, constant in member count**. |
| `InboundEmailAttachmentService` | `EmailToLeadHandler` (`classify` + **`stageBytes`** — the byte CARRIER, no Content DML); **`AttachmentPersistQueueable`** and **`AttachmentCarrierSweepBatch`** (`persist` + `linkTo` — the ONLY callers); `ExtractAddressQueueable` (`releaseCarrier`) | Broker Protection: the **only class in the app that writes `ContentVersion`, `ContentDocumentLink` or the classic `Attachment` byte carrier** for this module. 🔴 **2026-08-05/06 — v1 SHIPPED, CAUSED A LIVE OUTAGE, WAS REVERTED, AND WAS REDESIGNED (v2, 2026-08-06). THE INVARIANT THAT USED TO STAND IN THIS ROW — "`persist(...)` AND `linkTo(...)` MUST NOT THROW, under any input" — WAS FALSIFIED IN PRODUCTION AND HAS BEEN DELETED. Do not restore it and do not restore the call on the strength of a `try/catch`.** Measured on `usman-dpeg` via a four-case anonymous-Apex reproduction (full writeup in `EmailToLeadHandler`'s class header): `Database.insert(versions, false)` inside `persist()` threw `System.UnexpectedException: ContentPublication Limit exceeded` once the org's rolling **2,500-per-24h `ContentPublication` quota** was exhausted. That exception (a) **ignores `allOrNone = false`**, (b) **escapes `catch (Exception e)` through TWO nested layers**, arriving as `FATAL_ERROR`, and (c) triggers a **total rollback**, including the already-committed `Inbound_Email_Staging__c` insert — so **every inbound broker email carrying an attachment was silently destroyed**: no staging row, no Lead, no claim, no Task, no `AsyncApexJob`, no bounce, no error record. Attachment **size was irrelevant** (100 KB / 1 MB / 5 MB failed identically; heap peaked at 4,262 bytes), so this is categorically NOT the heap `LimitException` `VISION_MAX_BYTES` guards, and **"a better catch" is measured-dead, not a fix to propose.** **THE RULE THAT REPLACES THE OLD INVARIANT IS ABOUT CALLERS, NOT ABOUT CODE IN THIS CLASS: a `ContentVersion` insert must never sit inside a transaction whose rollback would lose something irreplaceable** — so `persist`/`linkTo` may be called ONLY from `AttachmentPersistQueueable` and `AttachmentCarrierSweepBatch`, never from `EmailToLeadHandler` (owns the email) and never from `ExtractAddressQueueable` (owns the Lead and the claim). **What the handler does instead (v2):** `stageBytes` parks each retained attachment on the staging row as a classic **`Attachment` byte carrier** — **zero ContentPublications (SP-4.1, `/limits` 2500 → 2500, definitive) and ~152 bytes of heap at ANY file size (SP-4.3, measured identically at 3 MB and 5 MB, because `Body = <blob>` is a reference assignment)** — in ONE bulk statement, pinning the handler at **0 SOQL / 2 DML (1 with no attachments)**. Constants moved with the redesign: `MAX_ATTACHMENTS` **10 → 3** (user decision) and `ATTACHMENT_MAX_BYTES` **5,000,000 → 4,000,000** (aligned to `CONVERT_MAX_TOTAL_BYTES` so any retained file always fits ONE conversion transaction by itself); `TEXT_MAX_BYTES` keeps its value but its **venue moved to the synchronous side**, so it now guards the 6 MB budget rather than the 12 MB one. `persist`'s per-row heap re-check was widened from the CSV branch to **every** row and re-based on `CONVERT_HEAP_CEILING`, because SP-4.5 measured `VersionData = att.body` on a SOQL-read body allocating the bytes a SECOND time (**peak ≈ 2 × converted bytes; one 5 MB file reached 10,487,036 of the 12,000,000 async ceiling**). Unchanged and still correct: `ShareType` `'I'` on routed **Lead / Opportunity / Contact** links and `'V'` on the staging link (an *edit* grant on an audit row must not confer the right to replace a broker's own submission); `'C'` is rejected by the platform everywhere; **no dedupe, deliberately** (decision E1) — a redelivery costs zero publications only because `ExtractAddressQueueable.execute` returns WITHOUT calling `finish()`, which is where the file job is enqueued, and that skip path now also RELEASES the duplicate's carrier; and **`LeadFileCarryOverService` still does NOT exist and must not be built** (spike S6 — the platform already carries a Lead's files to Account, Contact and Opportunity on `Database.convertLead`, so `LeadConvertService`'s `2 SOQL / ≤5 DML` contract and `LeadConvertTrigger` are untouched — ⚠ this sentence said `3 DML` until 2026-08-17; the FIGURE was stale, the CLAIM it makes is not). ⚠ **The carrier is TEMPORARY, with four defined exits** — CONVERTED (`AttachmentPersistQueueable`, only on confirmed success), RELEASED (`ExtractAddressQueueable`, on a gate or duplicate), RETRIED and EXPIRED (both `AttachmentCarrierSweepBatch`). See `agent-output/design-requirements-attachment-persistence-v2.md` and `agent-output/spike-attachment-persistence-v2.md`. |
...
| `PropertyClaimService`        | `ExtractAddressQueueable`                          | Broker Protection: owns all `Property_Registry__c` / `Competing_Broker_Submission__c` DML — acquires the `Property_Claim_Lock__c` FOR UPDATE lock, then registers a winner or marks a duplicate. **Sole owner of the WINNER ANCHOR rule (2026-08-03):** `buildSubmission` is the one place that decides Lead-vs-Opportunity anchoring, and `markDuplicate` / `logRepeatSubmission` take the caller's already-resolved live record so the rule costs no extra SOQL on the branches that fire. **⚠ AMENDED 2026-08-17 — THERE ARE NOW THREE `Property_Registry__c` DML STATEMENTS, NOT TWO, AND THE CLASS HEADER'S PROHIBITION PARAGRAPH WAS REWRITTEN IN PLACE RATHER THAN LEFT STANDING.** The third is `registerWinner`'s EXPIRED-adoption leg (broker-protection window): it re-points a registration whose window has closed onto the new claimant, resetting `Registered_DateTime__c`. 🔴 **It is admissible for exactly one reason** — `PropertyRegistrySelector.selectExpiredByPropertyKey` filters `Winning_Lead__r.IsConverted = false`, so the row it updates provably holds an OPEN Lead. **The prohibition still stands, unchanged, for any registry update NOT carrying that filter** (a "last touched" stamp, a status field, a re-key, a "superseded" marker, a sweep job). ⚠ **No Apex test in this org can falsify any of it** — `platform_lookupToAConvertedLead_isNotEnforcedInTestContext` pins that test context does not enforce the converted-Lead restriction, so a green suite proves the DECISION and never the platform rule. ⚠ The orphan and expired legs share ONE private `adopt(...)` method **deliberately**: two adoption paths writing two different field sets is how `Registered_DateTime__c` would acquire two meanings, which is the one thing both the fuzzy lookback and the expiry depend on it not doing. 🔴 **RESIDUAL R1 — the superseded broker's follow-ups do NOT become competing claims for up to `LOOKBACK_DAYS`**: `ExtractAddressQueueable`'s branch (b) REPEAT runs before branch (d) COMPETING and never consults the registry, so a handover leaves the old broker's later emails filed as Repeats on their own now-claimless Lead, silently. Accepted — the email is still captured and auditable, and fixing it means making the pipeline's highest-traffic branch registry-aware. |
| `BrokerLeaderboardService` | `BrokerLeaderboardController` (← `lwc/brokerLeaderboard`, Acquisitions Broker Hub) | **The broker leaderboard — who is actually bringing DPEG deals** (added 2026-08-17, FSD §27.3 / §27.4). **BY BROKER PERSON, not by firm** (user decision); the identity key is `Competing_Broker_Submission__c.Broker_Email__c`. 🔴 **COMPUTED LIVE, AND STORED ROLLUPS ON `Contact` ARE STRUCTURALLY IMPOSSIBLE RATHER THAN MERELY WORSE.** Since 2026-07-31 a COMPETING broker receives no Lead at all, so no conversion, so no Contact, ever — a Contact-based rollup could only ever describe the subset that WON **and** CONVERTED, i.e. it would silently omit every losing broker, which is most of the population a leaderboard is about. `Competing_Broker_Submission__c` also has **no Contact lookup** (the identity is Text), so there is no roll-up summary to build at any price. ⇒ **the six dead Contact broker-stat fields are left exactly as they are and this feature neither reads nor writes them.** 🔴 **THE SPLIT WINNER ANCHOR DOES NOT BITE — and saying so is the point.** Both aggregates `GROUP BY Broker_Email__c` and read NEITHER `Winning_Lead__c` nor `Winning_Opportunity__c`, so a converted winner's rows are counted like any other; contrast `CompetingBrokerSubmissionSelector.selectByWinningLead`, which reads one anchor and is blind to exactly those rows. ⚠ **It DOES bite the moment a drill-through or a deal-outcome column is added** — both anchors must then be read, and that Lead-only gap is still open. **Deals won / lost by OPPORTUNITY OUTCOME is deliberately NOT built**: it needs both anchors plus a conversion hop, and "did DPEG close it" is a different question from "did this broker win the property", which `Is_Winning_Submission__c` answers exactly. ✅ §27.4's *First Submission Date* falls out free as `MIN(Submitted_DateTime__c)` — no field, no writer, no backfill. ⚠ `MAX(Broker_Name__c)` is **deterministic-arbitrary**, a display hint only; the row's identity is the email. ⚠ **RESIDUAL — the blast-platform identity collapse is DISPLAYED, not suppressed**: ARCHITECTURE records U1 keying every broker who submits via RCM / Crexi / Buildout to the platform's own address, so the top row may be a PLATFORM rather than a person. Suppressing it would hide real volume and disagree with the ledger; a name/domain heuristic here would be a second, unowned copy of an arbitration rule. The component's help text names the shape, and U1's eventual platform-sender list repairs this surface for free. Budget: **2 SOQL / 0 DML, CONSTANT in broker count**, pinned at 251 brokers. Both aggregates are `WITH USER_MODE` on `with sharing` — the object is `sharingModel = ReadWrite`, so no automation-path exception and no `without sharing` inner class is warranted, and a human asked for this read, so a permission gap SHOULD throw rather than under-report a ranking. |
...
### 3.5 Placer.ai / CoStar — NOT an integration, and deliberately NOT a §3 exception

**There is no Placer.ai or CoStar integration in this application, and FSD tranche 2 (2026-08-16) did
not create one.** No callout, no `Http.send`, no Named Credential, no External Credential, no
scheduled pull, no ASB spoke. The market-data work delivered in that tranche is **§18.4's
"Integration Off" FALLBACK ONLY**: hand-entered fields on `Property__c` (the LIVE layer) plus an
Apex-composed freeze onto `Opportunity` at underwriting approval (`MarketDataSnapshotService`, §2).

⇒ **THIS SECTION EXISTS TO STOP A FUTURE READER COUNTING A THIRD EXCEPTION THAT DOES NOT EXIST.**
§3.1's ASB-only rule stands unqualified here, because nothing crosses a network boundary. The
standing exception count remains **two** (§3.3 OpenAI, §3.4 Microsoft Graph), and the review owed
above is not widened by this tranche.

🔴 **WHEN THE SPOKE LANDS, IT IS A DATA-SOURCE FLIP AND NOT A REBUILD — that is the property the
two-layer split was bought to preserve.** The spoke populates the SAME `Property__c` fields, sets
`Placer_Data_Source__c` / `CoStar_Data_Source__c` to `Integrated` and stamps the matching
`*_Last_Synced_DateTime__c`. Nothing downstream changes: the manual layer, the record page, the
snapshot composer and the freeze are all provider-agnostic. ⚠ At that point — and only then — a
callout service arrives and §3 gains a genuine entry, which under §3.1 belongs on **ASB** rather than
as a third direct exception.

⚠ **`Opportunity.Placer_Last_Synced_DateTime__c` / `CoStar_Last_Synced_DateTime__c` are a DIFFERENT
thing from the `Property__c` fields of the same name and must not be read as the live sync markers.**
They belong to the `lwc/marketDataSync` cards, whose Sync button stamps a DateTime and **contacts
nothing** (that bundle's own header states this at length). The authoritative freshness markers for
the market layer are the `Property__c` ones.
...
- ⚠ **A PASSIVE HOME-PAGE CARD USES AN INLINE `role="alert"` BANNER, NOT A TOAST.** A toast is
  correct for a failure the user CAUSED by clicking; a card the user merely scrolled past would
  raise an unprompted toast that then vanishes, leaving a permanently empty card with no
  explanation. Every `.lv-*` list card on the Acquisition home page follows this (`recentLeads`,
  `recentOpportunities`, `topLargestDeals`), and each also HIDES its datatable on the error branch
  rather than leaving stale rows on screen asserting data the server just refused to confirm.

### Two home-page deal cards — `recentOpportunities` and `topLargestDeals`

Added 2026-08-16 (FSD tranche 2, §24.2). **⚠ AMENDED 2026-08-17: `c/topLargestDeals` WAS RE-RANKED
ONTO `Asking_Price__c` AND CUT TO TWO COLUMNS, BY USER DECISION. Both cards now rank on the SAME
FIELD, so the separation argument this section used to make is largely spent — read the amendment
before citing it.** The Acquisition home page carries two five-row deal cards:

| Bundle | Ranks by | Stage scope | Columns | Apex |
| --- | --- | --- | --- | --- |
| `c/recentOpportunities` | `Asking_Price__c` | **every stage**, closed deals included | Deal Name, Stage, Deal Type, Asking Price, NOI | `OpportunityFunnelController.getRecentOpportunities` → `OpportunitySelector.selectTopByAskingPrice` |
| `c/topLargestDeals` | `Asking_Price__c` | **ACTIVE only** — `CLOSED_STAGES` excluded, which IS FSD §24.2 | **Property Name, Asking Price** | `TopDealsController.getTopDeals` → `OpportunitySelector.selectTopActiveByAskingPrice` |

⚠ **RETRACTED, verbatim, so nobody restores the design from the retired argument:** *"| `c/topLargestDeals` | **`Amount`** | … what the DEAL is worth to DPEG |"* and *"A deal can top one
list and not the other, so a merged card would silently answer only one of two questions while
appearing to answer both."* The ranking difference those sentences rested on **no longer exists**.

🔴 **WHAT STILL SEPARATES THE TWO CARDS IS ONE AXIS — THE CLOSED-STAGE EXCLUSION — AND IT IS THE FSD
REQUIREMENT ITSELF, NOT A STYLE CHOICE.** So the *conclusion* of the 2026-08-16 decision survives
even though its argument did not: **adding a stage filter to `selectTopByAskingPrice` and relabelling
the existing card is still not available**, because that method's own caller shows deals at ANY
stage. `recentOpportunities`, its controller method, its selector method, its label and its placement
remain UNTOUCHED. ⚠ Anyone proposing to merge the two cards is now proposing to drop one of *"the
five biggest active deals, any stage except Closed"* (§24.2) or the all-stage list — that is a
product decision, and it is a genuinely closer call than it was on 2026-08-16.

🔴 **THE SELECTOR METHOD WAS RENAMED WITH THE RANKING** — `selectTopByAmount` →
`selectTopActiveByAskingPrice` — rather than left naming a field it no longer orders by. It had
exactly ONE production caller, so the rename cost one call site; a sibling method would have left the
old one with zero callers and a live-looking Javadoc. This is the `LoiSelector` / `NdaSelector`
stale-header defect recorded in §2, caught before it was created rather than after.

⚠ **"Property Name" on `topLargestDeals` is `Opportunity.Name`, RELABELLED — not `Property__r.Name`**
(user decision, 2026-08-17). `LeadConvertService` names the deal after the property, so `Name`
already IS the property name; traversing would add a cross-object CRUD/FLS dependency under
`WITH USER_MODE` (which throws for the WHOLE ROW rather than blanking a column) and would render an
empty cell on any deal with a null `Property__c`. `OpportunitySelector`'s call-for-offers block
comment records the identical decision for that surface.

⚠ **THE `Opportunity.Amount` FLS NOTE IS NOW HISTORICAL.** `Amount` is `permissionable`, and on
2026-08-16 it was granted by no permission set in this repo (it appeared only in `profiles/**`, which
is `.forceignore`d and never deploys); the remedy — readable on `DPEG_Acquisition_Edit` / `_View` /
`DPEG_Opportunity_View` — was applied and verified IN SOURCE on 2026-08-17. 🔴 **The two-column cut
then removed `Amount`, `Deal_Type__c` and `Asset_Type__c` from that query entirely**, so this card's
`WITH USER_MODE` FLS surface is now the single field `Asking_Price__c` — already proven in production
by `selectTopByAskingPrice`. Do not infer that the `Amount` grants may be dropped: this class is not
their only possible consumer (layouts, list views and org-side reports name fields directly), and a
`PermissionSet` deploy REPLACES its whole `<fieldPermissions>` set.

⚠ **Stage/deal-type pill colours are INLINE styles built in JS, not `.css`,** and that is structural:
`c/listDatatable` is a `lightning/datatable` SUBCLASS, and a datatable subclass cannot reach its
custom cell templates from a stylesheet (see that bundle's own comment). Moving them into the `.css`
file silently stops applying them. The SLDS 2 tokens govern everything a stylesheet does reach.
⚠ **This now applies to `recentOpportunities` ONLY** — `topLargestDeals` lost both its pill columns
in the two-column cut, and its `STAGE` / `DEAL_TYPE` maps and `pillWrap` / `pillDot` builders were
deleted with them rather than left unreferenced.
...
### Parameterised record-page cards — `source` + `CONFIG_BY_SOURCE` (added 2026-08-16)

`c/marketDataSync` backs the **Placer** and **CoStar** cards on the `Opportunity_Record_Page` Detail
tab from ONE bundle, keyed by a `source` design property against a module-level `CONFIG_BY_SOURCE` map
holding the title, icon, field list and stamp field. This **extends the `advanceRecordStage`
`CONFIG_BY_TYPE` precedent from headless quick actions to RENDERED components**, and it carries the
same anti-duplication rule the subsection above records: `transactionAdvanceStage` was built and
deleted the same day for being a copy that differed only in its header. **Two cards that differ in
DATA and not in BEHAVIOUR are one bundle**; a third vendor is one map entry, not a third file.

⚠ **Field API names must arrive as `@salesforce/schema/...` imports, never as free-form strings in a
design property.** That is the load-bearing half. A comma-separated `fieldNames` property was
considered and rejected: it makes a renamed or deleted field deploy GREEN and render an empty card,
whereas a schema import makes it a **build-time** failure. It would also make the component droppable
on any object, which is a governance loss for no gain.

🔴 **A `targetConfig` `<property>` CANNOT be constrained to an enum**, so a parameterised component
MUST validate its key at runtime and render a visible inline `role="alert"` naming the accepted
values — derived from the map's own keys, so the message cannot drift from what the component
actually supports. A silently empty card is indistinguishable from "this record has no data", the
same trap §2 records for `DealFolderSweepBatch`'s all-zeros summary. ⚠ The lookup is **case-sensitive
and exact**, so the FlexiPage's `<value>` must match the map key byte for byte; the Jest suite covers
an unknown vendor, a blank, **and a case typo** for that reason.

⚠ Note also the `<objects><object>…</object></objects>` **element** form in `targetConfigs` — the
`objects=` *attribute* form is rejected by the Metadata API.

### The LDS-write / no-`getRecordNotifyChange` pairing extends to rendered cards

The guard-util table above states this rule for the quick-action bundles, but it is a property of the
**write mechanism, not of headlessness**. A rendered card writing through `updateRecord` must **NOT**
call `getRecordNotifyChange` — the write goes THROUGH the LDS cache, so the page re-renders on its
own. `c/marketDataSync` is the first such card and pins it with a dedicated Jest test asserting
**zero** invocations, **mutation-proved** to go red when the call is added (a `.not.toHaveBeenCalled()`
nobody has watched fail is worth very little). When such a component's write later moves to imperative
Apex — and a callout must be Apex — that test **inverts rather than disappearing**.

⚠ **Two LDS error reducers now exist in this repo and they are a deliberate copy, not an oversight**
(`c/leadStatusChange` and `c/marketDataSync`). An LDS write failure surfaces at
`error.body.output.errors[]` / `error.body.output.fieldErrors{}` and **`body.message` is often EMPTY**,
so the `body.message`-only read in `c/dealActionGuard` / `c/recordStageGuard` — correct for their Apex
paths — must never be copied to an LDS path. `c/marketDataSync` does not import
`c/leadStatusChange`'s copy because that module is Lead-bound by contract and would drag
`LeadActionPermissionController` into a card designed to hold no Apex dependency. Extracting both into
`c/utils` as a shared `ldsErrorMessage` was reviewed on 2026-08-16 and **deliberately deferred** (user
decision) rather than rejected: it touches a live, deployed bundle. ⇒ Whoever next edits either
reducer extracts both, per §6.

### FLS as the gate, where there is no privileged operation

Where a button's entire effect is writing one field the user could already type by hand, it needs
**no custom permission and no `<visibilityRule>`** — FLS edit on that field IS the correct gate and is
already modelled in the module's permission sets, at zero new metadata and with no layer-4/layer-5
placement decision to get wrong. Contrast the three stage-action gates, every one of which guards a
record moving through a business process under server-side logic.

The component reads the field's `updateable` flag from `getObjectInfo` and renders the button
**disabled with a reason** — never hidden, never left to throw on click. 🔴 **The check is THREE-state
(`true` / `false` / not-yet-known) and the two halves must diverge**: the disable folds "unknown" into
disabled (fail closed), while the reason text does NOT, so a still-loading card does not accuse the
user of lacking access for the sub-second the wire is in flight.

🔴 **The general rule the 2026-08-16 review extracted from this, which is the part worth carrying:
a card must never assert a cause it has not established.** Three separate defects of that one shape
were found and fixed in this bundle's review — a `getObjectInfo` failure reported as an FLS denial (it
is more plausibly object access or a transient failure), and a `Last Synced` row rendering **"Never"**
during the wire's loading window (telling the user the record has never been synced *before it
knows*). The correct outputs are "not available" and a placeholder respectively. **Every empty-state
label on a wire-backed card needs a distinct answer for absent, unknown-yet, and failed-to-read** —
collapsing any two of them produces a confident wrong answer, which is worse than a blank.
