...
As of 2026-08-06 the repo has **20 `WITH SYSTEM_MODE` queries across 13 selector classes** (up from
17 across 12: `AttachmentSelector` is new and contributes two, and
`InboundEmailStagingSelector.queryCarrierSweep` is the third — that class now mixes both modes
deliberately, so read the method, not the class):
