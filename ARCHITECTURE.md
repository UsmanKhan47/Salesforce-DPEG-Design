...
As of 2026-08-09 the repo has **23 `WITH SYSTEM_MODE` queries across 14 selector classes** —
`NdaSelector` is the newest entrant and contributes exactly one (`selectByDispositionIds`) against
its four `USER_MODE` methods, so its A1-era header claim "no SYSTEM_MODE / guest path" is now false
and has been amended in place. As of 2026-08-08 it was **22 across 13** (up from
...
sharing `c/recordStageGuard`. The BRANCH targets get their own one-action bundles rather than a
