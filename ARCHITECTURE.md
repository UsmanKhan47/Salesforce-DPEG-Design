...
**†** marks a row that carries _Purpose_ prose despite the object having **no `<description>`**. That text is **authored inference, not condensed source** — do not cite it as authoritative; write the `<description>` and then condense it here. The arithmetic reconciles: 19 dashed rows + 3 inferred rows (†) = the 22 unset. (It was 20 + 2 until Tranche 5A gave `Property_Asset__c` an automated creator and therefore a † row; its `<description>` is still unset and writing it is admin work, so the count of unset objects is unchanged at 22.)
...
As of 2026-08-11 (SharePoint deal folder) the repo has **31 `WITH SYSTEM_MODE` queries across 19
...
contributing exactly one — `selectByDispositionIds` — against its four `USER_MODE` methods, so its
A1-era header claim "no SYSTEM_MODE / guest path" is now false and has been amended in place). As of
...
| `DispositionActionPermissionService` | `RecordStageAdvanceService` (via the per-record-type gate dispatch) | **The DISPOSITION stage-action gate — the sell-side twin of `OpportunityActionPermissionService`** (added 2026-08-09, D15/Q1). TWO-FACTOR: FLS read on `User.Disposition_Driver__c` (granted only by the `Disposition_Deal_Driver` permission set, enforced by `WITH USER_MODE` in `UserSelector`, whose throw IS the denial signal) **AND** that flag `true` on the running user. 🔴 **The "Modify All Data" bypass runs BEFORE the flag read, and reversing those two statements is a lockout, not a style choice** — a Metadata-API-deployed custom field arrives with no field permissions for ANY profile, System Administrator included, so the selector throws for a bare admin exactly as for a bare Standard User. Three cheaper options were rejected and each looks like tidying: granting `Deal_Driver__c` to disposition users (merges two authorization boundaries — a sale driver could then drive acquisitions), a permission-set MEMBERSHIP check (replaces a two-factor condition with a one-factor one for every holder), and no gate at all. Reads via `UserSelector` + `PermissionSetAssignmentSelector`; no DML. It reuses `OpportunityActionPermissionService.NO_PERMISSION_MESSAGE` rather than re-declaring it, so the two gates can never speak with two voices. ⚠ **`UserSelector` keeps the two flag reads as SEPARATE queries on purpose** — a merged `SELECT Deal_Driver__c, Disposition_Driver__c` would make `USER_MODE` throw for an acquisitions-only driver, and the service converts a throw to `false`, silently denying a legitimate deal driver the actions they have always had. |
| `TransactionActionPermissionService` | `RecordStageAdvanceService` (via the per-record-type gate dispatch, `StageActionGate.TRANSACTION_STAGE_ACTIONS`) | **The TRANSACTION stage-action gate** (added 2026-08-12 with the Transaction Advance Stage action). 🔴 **IT IS MEMBERSHIP-ONLY, AND IT IS THE FIRST GATE IN THIS TABLE THAT IS NOT TWO-FACTOR — do not read the two `*_Driver` rows above and assume the shape.** One factor: membership of `Transaction_Stage_Actions_Access` (directly or through a permission set GROUP), with a "Modify All Data" bypass. It follows `LeadActionPermissionService`, not its two stage-gate siblings, and delegates to that class's public `hasAnyPermissionSet(Set<String>)` rather than copying the logic — which buys group expansion, a SHARED transaction-scoped cache (so this gate costs zero extra queries when anything else has already asked) and one "Modify All Data" computation instead of two caches that can disagree about the same user. 🔴 **WHY THE SIBLINGS' TWO-FACTOR ARGUMENT DOES NOT TRANSFER, which is the whole justification and is HISTORICAL:** on `Opportunity`, `User.Deal_Driver__c` **already existed and already drove live Dynamic Actions visibility rules** — `Opportunity_Record_Page` carries **eight** `{!$User.Deal_Driver__c}` references (measured 2026-08-12) — so a membership check there would have **WIDENED a live authorization boundary**, granting every set-holder whose flag is `false`. `Transaction__c` has **no such flag and no such population**: there is no `User.Transaction_Driver__c`, no visibility rule bound to one, and therefore nobody whose access membership could widen. ⇒ **The two-factor pattern protects an EXISTING boundary; where there is none it is a second moving part with a silent failure mode** (two undeployable org steps — assign the set AND tick the box — whose omission just makes the action refuse, with no error naming the missing tick). USER DECISION 2026-08-12, overriding the design document's `TRANSACTION_DRIVER` recommendation. ⚠ **The enum member is deliberately NOT named `TRANSACTION_DRIVER`** — a `*_DRIVER` name would assert a flag a reader would then go looking for. ⚠ **A CONSEQUENCE THAT MUST BE STATED RATHER THAN DISCOVERED: the gate is TEAM-WIDE.** `Transaction_Stage_Actions_Access` is a member of the `DPEG_Transaction_Team` permission set group, and a group's membership is the unit of assignment — so **per-user revocation is not available**: denying one user means removing them from the group and losing its other ten sets with it. The placement itself is precedented (`Opportunity_Stage_Actions_Access` sits inside `DPEG_Junior_Analyst_PSG` identically); what Opportunity's per-user granularity actually comes from is its layer-5 flag set, assigned DIRECTLY, which this gate has none of by decision. That need is the trigger to add a flag later, argued from the need and never from symmetry. ⚠ **No FLS read anywhere**, so unlike the two `*_Driver` gates there is no ordering hazard and no risk of locking out the administrator who just deployed the feature. Reads via `LeadActionPermissionService`; no SOQL and no DML of its own. Test seams (`clearCache()`, `simulateLoadFailure`) live on `LeadActionPermissionService` and are deliberately not re-declared. |
...
**⚠ BUT A LAYER-4 SET DOES NOT REQUIRE A LAYER-5 PARTNER, AND THE PARAGRAPH ABOVE SHOULD NOT BE READ
AS SAYING IT DOES (added 2026-08-12).** `Transaction_Stage_Actions_Access` is a layer-4 capability set
with **no layer-5 counterpart and deliberately none** — `TransactionActionPermissionService` gates on
MEMBERSHIP of that set alone, with a "Modify All Data" bypass and no `User` checkbox anywhere. That is
not a violation of the split; it is the split applied where only one of the two questions exists.
**The rule that actually governs is narrower than "always pair them":** where a `User.*` flag ALREADY
EXISTS and already drives live access, the flag read must not be replaced by a membership check,
because that grants every set-holder whose flag is `false` — a silent widening of a live boundary
(eight `{!$User.Deal_Driver__c}` visibility references on `Opportunity_Record_Page`, measured
2026-08-12, are what make this concrete on the acquisition side). Where **no flag and no population
exist**, there is nothing to widen, and inventing a flag so the shapes match adds a second undeployable
org step — assign the set AND tick the box — whose omission fails silently, the action simply refusing
with nothing naming the missing tick. ⇒ **Pair a capability set with an authorization flag when the
flag is protecting something; do not create a flag to satisfy symmetry.** The decision to add one later
should be argued from a stated need (see the next paragraph for the concrete need that would trigger it
on Transactions), never from the shape of a sibling.

**⚠ A capability set placed inside a persona GROUP is TEAM-WIDE, and where it has no layer-5 partner
that is the whole granularity — state it rather than let a reader infer per-user control.**
`Transaction_Stage_Actions_Access` is a member of `DPEG_Transaction_Team`, so the Transaction stage
gate answers "is this user on the Transactions team?" and **per-user revocation is not available**:
denying one user means removing them from the group and losing its other ten sets with them. The
placement is precedented and is not the problem — `Opportunity_Stage_Actions_Access` sits inside
`DPEG_Junior_Analyst_PSG` in exactly the same way. The difference is that Opportunity's per-user
granularity comes from its layer-5 flag set (`Acquisition_Deal_Driver`), which is assigned **directly**
rather than through the group; the capability set's placement never provided it there either. A need to
deny one user while keeping them on the team is therefore the concrete trigger to add a layer-5 flag —
and no edit to any permission set achieves it without one.
...
| Gate shape behind it | membership (`LeadActionPermissionService`) | TWO-FACTOR — permission set **AND** `User.Deal_Driver__c` | **MIXED, and the client cannot tell**: `DEAL_DRIVER` / `DISPOSITION_DRIVER` are two-factor, **`TRANSACTION_STAGE_ACTIONS` is membership-only** (`Transaction_Stage_Actions_Access`, no `User` flag). This is exactly what the per-record signature buys — a third gate of a NEW SHAPE landed as server config and touched no client file |
