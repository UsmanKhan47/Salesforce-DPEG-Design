/**
 * c-deal-move-to-about-to-close — IMPERATIVE APEX (headless quick action).
 * ------------------------------------------------------------------------
 ...
 *   const message = await advanceTo({ recordId, target: 'About to Close' });
 * On success it toasts the returned message and requests an LDS refresh via getRecordNotifyChange;
 * on failure it toasts the Apex error (or a fallback).
 *
 * Follows the imperative-Apex template (c-submit-for-approval): the imported Apex method is a bare
 * jest.fn driven with mockResolvedValue / mockRejectedValue, and the observable output is the
 * dispatched ShowToastEvent (empty template = no DOM), captured with addEventListener.
 * getRecordNotifyChange is auto-mocked as a jest.fn by the sfdx-lwc-jest lightning/uiRecordApi stub.
 *
...
import DealMoveToAboutToClose from 'c/dealMoveToAboutToClose';
...
describe('c-deal-move-to-about-to-close', () => {
    beforeEach(() => {
    ...
    function createComponent(props = { recordId: RECORD_ID }) {
        const element = createElement('c-deal-move-to-about-to-close', {
            is: DealMoveToAboutToClose
        });
    ...
    // A macrotask, not a bare microtask: invoke() awaits the guard (permission promise -> confirm
    // promise) before the Apex promise, so a single Promise.resolve() would not drain the chain.
    const flushPromises = () => new Promise((resolve) => setTimeout(resolve, 0));

    it('SUCCESS BRANCH: moves the deal to About to Close and toasts the Apex message', async () => {
        advanceTo.mockResolvedValue('Deal moved to About to Close.');
        ...
        // The exact target string matters twice over: it must match the picklist's DECODED runtime
        // value ('About to Close'), and it must be a member of StageAdvanceService's explicit-target
        // allow-list or the Apex now refuses it.
        expect(advanceTo).toHaveBeenCalledWith({
            recordId: RECORD_ID,
            target: 'About to Close'
        });
    ...
    it('CONFIRM: asks the About to Close question before calling Apex', async () => {
    ...
    it('is accessible', async () => {
        ...
        // Headless action: an empty shadow root is trivially accessible, but the assertion is kept
