/**
 * Same IMPERATIVE APEX pattern as c-nda-mark-declined, and the same reason for pinning the exact
 * argument pair rather than "Apex was called": TARGET_STAGE is a module constant precisely so
 * RecordStageAdvanceService can validate it against the DISPOSITION LOI's per-record-type
 * allow-list. A bundle that varied its own target would defeat that check, so the first test
 * asserts the payload, not the call.
 *
 * Uses the REAL c/recordStageGuard, so the guard's two dependencies are mocked here:
 * lightning/confirm (whose real stub throws on .open() by design) and the permission Apex (an
 * un-mocked apex import resolves undefined, which the guard's `=== true` check reads as DENIED and
 * would redden every happy path).
 *
 ...
 * proves this bundle sends the right thing and reacts correctly — never that an acquisition LOI is
 * refused. RecordStageAdvanceServiceTest.acquisitionLoiRefusesADispositionStageAsAnExplicitTarget
 * is the test that proves the refusal, and dispositionLoiAcceptsTheNegotiationLoopTargets is the
 * complement that stops it passing vacuously.
 */
import { createElement } from 'lwc';
import LoiMarkCounteredByDpeg from 'c/loiMarkCounteredByDpeg';
...
describe('c-loi-mark-countered-by-dpeg', () => {
    beforeEach(() => {
    ...
    function createComponent() {
        const element = createElement('c-loi-mark-countered-by-dpeg', {
            is: LoiMarkCounteredByDpeg
        });
    ...
    it('SUCCESS BRANCH: sends the hardcoded back-edge target, toasts success, notifies the record', async () => {
        advanceTo.mockResolvedValue('LOI moved to Countered by DPEG.');
        ...
        // allow-list check downstream is only meaningful if the client cannot vary it. This is
        // also the LOOP'S BACK-EDGE, the one transition the linear NEXT_STAGE map cannot derive.
        expect(advanceTo).toHaveBeenCalledTimes(1);
        expect(advanceTo).toHaveBeenCalledWith({
            recordId: RECORD_ID,
            target: 'Countered by DPEG'
        });
    ...
    it('ERROR BRANCH: surfaces the Apex message verbatim, no record notify', async () => {
        // The exact refusal an acquisition LOI gets from the per-record-type allow-list.
    ...
    it('ERROR BRANCH: a body-less rejection falls back to the fixed message', async () => {
        ...
        expect(toastHandler.mock.calls[0][0].detail.message).toBe(
            'The LOI could not be moved to Countered by DPEG.'
        );
    });
